      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm h-fit border border-gray-100 flex flex-col justify-between">
          ...
          <div className="bg-gray-50 rounded-xl p-6 text-center border border-gray-100">
             ...
             <button
              onClick={onVote}
              className={`w-full flex items-center justify-center gap-3 px-6 py-4 rounded-xl font-bold text-lg transition-all transform active:scale-95 shadow-sm ${
                hasVotedForThis 
              ...
              <tr key={video.id} className="hover:bg-gray-50/50 transition-colors">
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <button 
                      onClick={() => {
                        const isPermanent = video.isHidden;
                        const msg = isPermanent 
                          ? "WARNING: This will permanently delete the video from the database. This cannot be undone." 
                          : "Confirm delete video? It will be removed from the public feed.";
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-500">Real-time voting analytics & content moderation</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 bg-white p-2 rounded-lg border border-gray-200 shadow-sm">
             <div className="px-4 py-2 text-center border-r border-gray-100">
                 <span className="block text-xs text-gray-400 uppercase font-bold">Total Videos</span>
                 <span className="text-xl font-bold text-gray-900">{videos.length}</span>
             </div>
             <div className="px-4 py-2 text-center">
                 <span className="block text-xs text-gray-400 uppercase font-bold">Total Votes</span>
                 <span className="text-xl font-bold text-indigo-600">{totalVotes}</span>
             </div>
        </div>
      </div>

      <VideoTable list={activeVideos} title="Live Public Videos" emptyMsg="No active videos." />
      
      <VideoTable list={hiddenVideos} title="Hidden / Archived Videos" emptyMsg="No hidden videos." />
...
const App: React.FC = () => {
  ...
  // Load initial data asynchronously (for IndexedDB)
  useEffect(() => {
    const init = async () => {
      try {
  ...
  const handleUploadSuccess = async (newVideo: Video, file: File) => {
    ...
    const updatedVideos = await getStoredVideos();
  ...
  const handleDeleteVideo = async (video: Video) => {
    try {
      const isSoftDelete = !video.isHidden;
      
      if (isSoftDelete) {
          // It's public, so Soft Delete (Hide)
          await toggleVideoVisibility(video.id, true);
      } else {
          // It's hidden (Admin view trash can), so hard delete
      ...
      // If we were viewing this video in details, go back to dashboard
      if (view === 'VIDEO_DETAIL' && selectedVideoId === video.id) {
        // 1. Go back immediately
        setView('DASHBOARD');
        
        // 2. Show feedback AFTER state change to prevent blocking
        setTimeout(() => {
          alert(isSoftDelete ? "Video has been removed." : "Video permanently deleted.");
        }, 100);

      } else {
         // Admin panel view or other
         alert(isSoftDelete ? "Video hidden from feed." : "Video deleted.");
      }
    } catch (error) {
  ...
  const handleVote = useCallback(() => {
    ...
    if (currentVote === selectedVideoId) {
        ...
        const updatedVotes = removeVote(user.id);
        setVotes(updatedVotes);
    } else {
        // Vote for this one (replaces old vote)
        const updatedVotes = castVote(user.id, selectedVideoId);
        setVotes(updatedVotes);
    }
